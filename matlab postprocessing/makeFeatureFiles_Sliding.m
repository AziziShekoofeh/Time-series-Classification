function [X_bmode, X_rf, infoROI, infoCore, PatientsInfo_FileName] = makeFeatureFiles_Sliding(path_1, path_2)
...
for i = 1 : size(PatientsInfo_FileName,1)
    filename = [path_2,'./features_rf_tsc_ROI_Sliding/feature_',PatientsInfo_FileName(i,:),'.mat'];
    load(filename);
    X = [X; feature]; %#ok<AGROW>
    size_samples = [size_samples; size(feature,1)]; %#ok<AGROW>
    ROI_num = 1:size(feature,1);
    infoROI = [infoROI; [repmat(PatientsInfo(i,:),[size(feature,1),1]),ROI_num']]; %#ok<AGROW>
end
X_rf = X;
...
feature = [];
for i = 1 : size(PatientsInfo_FileName,1)
    filename = [path_2,'./features_bmode_tsc_ROI_Sliding/feature_',PatientsInfo_FileName(i,:),'.mat'];
    load(filename);
    X = [X;feature]; %#ok<AGROW>
end
X_bmode = X;
